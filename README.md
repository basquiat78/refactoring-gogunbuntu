...
jpa를 어느정도 공부했던 이 주니어 개발자는 myBatis에서 enum의 경우에는 매핑이 되지 않는다는 것을 알고 좀 독특하게 처리했는데

myBatis에서는 이것을 해결하기 위해 TypeHandler를 제공하기 때문에 사실 이것을 정의하고 myBatis관련 config.xml에 정의를 하면 끝난다.
